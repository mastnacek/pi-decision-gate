// commands.ts — registrace příkazů /gate a /decision-gate pro pi-decision-gate
...
export function registerGateCommands(pi: ExtensionAPI): void {
  const registerOne = (cmdName: string) => {
    pi.registerCommand(cmdName, {
      ...
      getArgumentCompletions: async (prefix: string): Promise<AutocompleteItem[] | null> => {
        ...
        const getCompletionsClean = async (cleanPrefix: string): Promise<AutocompleteItem[] | null> => {
